...
type BlockChain struct {
	...
	validator  abstract.Validator // Block and state validator abstract
	prefetcher abstract.Prefetcher
	processor  abstract.Processor // Block transaction processor abstract
	vmConfig   vm.Config
...
func (bc *BlockChain) insertChain(chain types.Blocks, verifySeals bool) (int, error) {
	...
	defer func() {
		if lastCanon != nil && bc.CurrentBlock().Hash() == lastCanon.Hash() {
			bc.chainHeadFeed.Send(core.ChainHeadEvent{lastCanon})
		}
	}()
	...
	for ; block != nil && err == nil || err == core.ErrKnownBlock; block, err = it.next() {
		...
		if BadHashes[block.Hash()] {
			bc.reportBlock(block, nil, core.ErrBannedHash)
			return it.index, core.ErrBannedHash
		}
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	if len(deletedLogs) > 0 {
		bc.rmLogsFeed.Send(core.RemovedLogsEvent{mergeLogs(deletedLogs, true)})
	}
	...
	if len(oldChain) > 0 {
		for i := len(oldChain) - 1; i >= 0; i-- {
			bc.chainSideFeed.Send(core.ChainSideEvent{Block: oldChain[i]})
		}
	}
	return nil
}
...
func (bc *BlockChain) maintainTxIndex(ancients uint64) {
	...
	var (
		...
		headCh = make(chan core.ChainHeadEvent, 1) // Buffered to avoid locking up the event feed
	)
