...
func LibraryAddressFor(name string) (common.Address, error) {
	...
	if !ok {
		return params.ZeroAddress, fmt.Errorf("can't find genesis address for %s", name)
	}
	return address, nil
}
...
func ProxyAddressFor(name string) (common.Address, error) {
	...
	if !ok {
		return params.ZeroAddress, fmt.Errorf("can't find genesis address for %sProxy", name)
	}
	return address, nil
}
