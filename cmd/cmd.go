...
const (
	clientIdentifier = "atlas" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the map-protocol atlas command line interface")
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.SingleFlag.Name):
		log.Info("Starting Atlas on singlenet...")
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) != "light" && !ctx.GlobalIsSet(utils.CacheFlag.Name) && !ctx.GlobalIsSet(utils.NetworkIdFlag.Name) {
		...
		if !ctx.GlobalIsSet(utils.TestnetFlag.Name) && !ctx.GlobalIsSet(utils.DeveloperFlag.Name) && !ctx.GlobalIsSet(utils.SingleFlag.Name) {
...
func startNode(ctx *cli.Context, stack *node.Node, backend atlasapi.Backend) {
	...
	// Create a client to interact with local atlas node.
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.SingleFlag.Name) {
